...
class ResponseController {
  async lists(req, res) {
    try {
      ...
      return res.status(200).json({ status: true, message: "ANSWER_FOUND", form, total: form.answers.length, answers: form.answers });
    } catch (error) {
  ...
  async summaries(req, res) {
    try {
      ...
      return res.status(200).json({ status: true, message: "ANSWER_FOUND", summaries });
    } catch (error) {
      return res.status(error.code || 500).json({ status: false, message: error.message });
    }
  }
