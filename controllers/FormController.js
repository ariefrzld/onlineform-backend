...
class FormController {
  async index(req, res) {
    try {
      ...
      return res.status(200).json({ status: true, message: "FORMS_FOUND", total: form.length, form });
    } catch (error) {
  ...
  async update(req, res) {
    try {
      ...
      return res.status(200).json({ status: true, message: "FORM_UPDATE_SUCCESS", form });
    } catch (error) {
      return res.status(error.code || 500).json({
        status: false,
        message: error.message,
  ...
  async destroy(req, res) {
    try {
      ...
      return res.status(200).json({ status: true, message: "FORM_DELETE_SUCCESS", form });
    } catch (error) {
      return res.status(error.code || 500).json({
        status: false,
        message: error.message,
    ...
    try {
      ...
      if (req.jwt.id != form.userId && form.public === false) {
        ...
        if (!form.invites.includes(user.email)) {
          throw { code: 401, message: "YOU_ARE_NOTE_INVITED" };
        }
      }
